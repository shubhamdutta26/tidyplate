for (i in c(6, 12, 24, 48, 96, 384, 1536)) {
  ...
  test_that("view_plates works for complete valid data", {
    csv_file <- paste0(path, "allWellIds.csv")
    xlsx_file <- paste0(path, "allWellIds.xlsx")

    result_csv <- view_plates(csv_file)
    result_xlsx <- view_plates(xlsx_file)

    expect_identical(result_csv, "values")
    expect_identical(result_xlsx, "values")
  })

  test_that("view_plates works for missing data from plate", {
  ...
  test_that("view_plates works with one full plate and one partially empty", {
  ...
  test_that("view_plates returns a character vector", {
    expect_type(view_plates(paste0(path, "allWellIds.csv")), "character")
    expect_type(view_plates(paste0(path, "allWellIds.xlsx")), "character")
  })

  test_that("view_plates allows weird characters", {
  ...
  test_that("view_plates works with single well", {
  ...
  test_that("view_plates throws error when plate format is not correct", {
  ...
  test_that("view_plates throws error when row and column ids are empty or bad", {
  ...
  test_that("view_plates throws error when row id(s) are empty or bad", {
  ...
  test_that("view_plates throws error when column id(s) are empty or bad", {
...
test_that("view_plates throws an error when user inputs bad file type", {
  expect_error(view_plates(file = 1), "file argument cannot be of type: double")
  expect_error(view_plates(file = 1L), "file argument cannot be of type: integer")
  expect_error(view_plates(file = TRUE), "file argument cannot be of type: logical")
})

test_that("view_plates throws an error when user inputs more than one file", {
  file_1 <- "test_data/allWellIds.csv"
  file_2 <- "test_data/oneFullOnePartEmpty.csv"

  expect_error(
    view_plates(file = c(file_1, file_2)),
...
test_that("view_plates throws an error when input file is non existant", {
  csv_file <- "test_data/doesNotExist.csv"
  xlsx_file <- "test_data/doesNotExist.xlsx"

  expect_error(view_plates(csv_file), "doesNotExist.csv: Not OK; File does not exist.")
  expect_error(view_plates(xlsx_file), "doesNotExist.xlsx: Not OK; File does not exist.")
})

test_that("view_plates throws an error when input file is not csv or xlsx", {
  file <- "test_data/badFileType.txt"

  expect_error(view_plates(file), "badFileType.txt: Not OK; Must be either xlsx or csv file.")
})

test_that("view_plates throws an error when input file is empty", {
  csv_file <- "test_data/emptyFile.csv"

  expect_error(view_plates(csv_file), "emptyFile.csv: Not OK; File is empty.")
})
